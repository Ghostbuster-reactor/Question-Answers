...
SELECT json_build_object('question', '168280', 'page', 1, 'count', 5, 'results', ARRAY((SELECT json_build_object('answer_id', a2.id, 'body', a2.body, 'date', a2.date_written, 'answerer_name', a2.answerer_name, 'helpfulness', a2.helpful, 'reported', a2.reported, 'photos', (SELECT ARRAY_AGG (url) FROM answers_photos WHERE answer_id=a2.id)) FROM answers a2 WHERE question_id=168280 ORDER BY a2.helpful DESC)));

-- returns amazing questions object
SELECT json_build_object(
  'product_id', '40380',
  'results',
  ARRAY(
...
select count(*) from answers_photos where answer_id=6297785)=0 from answers_photos where answer_id=6297785;
--no photos 328948
